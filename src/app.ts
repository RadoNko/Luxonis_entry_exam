...
async function savePSQL(data: any) {
    // await pool.query('CREATE TABLE apartments_table(name text,address text,price int,image_url text)')
...
app.get("/", (req: Request, res: Response, next: NextFunction) => {
    /*
    (pool.query('SELECT $1::text as connected', ['Connection to postgres successful!'])).then((resp)=>{
        res.send(resp.rows[0].connected);
    });
     */
